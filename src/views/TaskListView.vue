...
// Переход сюда по ссылке с другим ?project= (например, клик в умном поиске
// шапки — SmartSearch.vue), когда страница уже открыта: сам компонент не
// перемонтируется, а projectId читается из query только при первом заходе —
// без этого watch-а URL меняется, а таблица остаётся отфильтрованной по-старому.
