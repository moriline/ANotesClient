...
**[Живое демо](https://moriline.github.io/ANotesClient/)** — без сервера,
данные сбрасываются при перезагрузке страницы.

> **Статус:** живое демо на GitHub Pages (клиент без бэкенда), в продакшене с реальным сервером не разворачивался. Подробности — в разделе [Состояние проекта](#состояние-проекта).
...
Без бэкенда под рукой — `npm run dev:demo` поднимает тот же интерфейс с демо-транспортом (без сети, данные в памяти вкладки): вход принимает любые логин/пароль, весь API подменяется в `src/api/http.ts` на in-memory-хранилище (`src/api/demo/`). Ровно это собирается и деплоится в [живое демо](https://moriline.github.io/ANotesClient/) — см. workflow `.github/workflows/demo.yml`.
...
| `npm run dev:demo` | дев-сервер в демо-режиме, без бэкенда |
| `npm run build` | сборка: `vite build`, затем `vue-tsc -b` |
| `npm run build:demo` | сборка демо для GitHub Pages (`base=/ANotesClient/`, hash-роутер) |
